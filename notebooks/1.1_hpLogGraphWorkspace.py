...
results = kt_logs_to_dfs(kt_path = '../data/atlas/models/Gpca_raw_k0/kt/')
...
# %%
csv_log_path = '../data/atlas/models/Gpca_raw_k0/logs/training_log.csv'
...
#     fig.update_yaxes(title_text="Log10 Loss", type="log",  row=2, col=1)
...
# %%
model_names = ['Gpca_k']#, 'Gpca_raw_k', 'Gnum_k', 
...
fig.show()
fig.write_html("../reports/211215_checkin/validation_performance.html")
...
fig.show()
fig.write_html("../reports/211215_checkin/validation_mean_by_sd.html")

# %% code_folding=[18, 26, 29, 34, 42, 52, 62] hidden=true
fig = make_subplots(rows=1, cols=1, subplot_titles=(""))
...
fig.write_html("../reports/211215_checkin/validation_selection_criteria.html")
...
fig.write_html("../reports/211215_checkin/best_cross_trials.html")
fig.show()
...
search_strs = []
for entry in ['Gpca_k', 'S_k', 'Wmlp_k', 'Wc1d_k', 'Wc2d_k']:
...
# %%
Gpca_hps = wrap_get_hp_info(match_str = 'Gpca_k')
Gpca_hps = desired_models.merge(Gpca_hps, how = 'inner')

# %%
S_hps = wrap_get_hp_info(match_str = 'S_k')
S_hps = desired_models.merge(S_hps, how = 'inner')

# %%
Wmlp_hps = wrap_get_hp_info(match_str = 'Wmlp_k')
Wmlp_hps = desired_models.merge(Wmlp_hps, how = 'inner')

# %%
Wc1d_hps = wrap_get_hp_info(match_str = 'Wc1d_k')
Wc1d_hps = desired_models.merge(Wc1d_hps, how = 'inner')

# %%
Wc2d_hps = wrap_get_hp_info(match_str = 'Wc2d_k')
Wc2d_hps = desired_models.merge(Wc2d_hps, how = 'inner')
...
# %%
wrap_hp_plot_by_model(plot_df = S_hps)

# %%
wrap_hp_plot_by_model(plot_df = Wmlp_hps)

# %%
wrap_hp_plot_by_model(plot_df = Wc1d_hps)

# %%
wrap_hp_plot_by_model(plot_df = Wc2d_hps)
...
write_trial_hps(
    df_in = S_hps, 
    save_path = '../data/atlas/models/S_finalize_hp/')

# %%
write_trial_hps(
    df_in = Wmlp_hps, 
    save_path = '../data/atlas/models/Wmlp_finalize_hp/')

# %%
write_trial_hps(
    df_in = Wc1d_hps, 
    save_path = '../data/atlas/models/Wc1d_finalize_hp/')

# %%
write_trial_hps(
    df_in = Wc2d_hps, 
    save_path = '../data/atlas/models/Wc2d_finalize_hp/')
...
# %% [markdown] heading_collapsed=true
# # Let's assume we wanted to select networks with similar tuning hps. What would that look like?
#
#

# %% hidden=true
list_of_hps = [Gpca_hps,
               S_hps,
               Wmlp_hps,
               Wc1d_hps,
               Wc2d_hps]
...
# %% hidden=true
# reduce down to just the variables that would be shared
for i in range(len(list_of_hps)):
    temp = list_of_hps[i]
    temp = temp.loc[temp.hyperparameters.isin(['batch_size','beta1', 'beta2', 'learning_rate'])]
    list_of_hps[i] = temp
...
for i in range(len(list_of_hps)):
    if i == 0:
        ...
        temp = temp.merge(list_of_hps[i], how = 'outer')

temp

# %% hidden=true
temp = temp.pivot_table(index = ['model', 'model_group', 'trial_id'], 
...
# %% hidden=true
import plotly.express as px
fig = px.scatter_3d(temp.loc[temp.learning_rate == 0.1000, ], 
                    x='batch_size', 
                    y='beta1', 
                    z='beta2',
                    color='model_group')
...
# %% hidden=true
fig = px.scatter_3d(temp.loc[temp.learning_rate == 0.0100, ], 
                    x='batch_size', 
                    y='beta1', 
                    z='beta2',
                    color='model_group')
...
# %% hidden=true
fig = px.scatter_3d(temp.loc[temp.learning_rate == 0.0010, ], 
                    x='batch_size', 
                    y='beta1', 
                    z='beta2',
                    color='model_group')
...
# %% hidden=true
fig = px.scatter_3d(temp.loc[temp.learning_rate == 0.0001, ], 
                    x='batch_size', 
                    y='beta1', 
                    z='beta2',
                    color='model_group')
