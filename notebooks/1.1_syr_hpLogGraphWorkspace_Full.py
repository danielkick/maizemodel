...
# %%
res = kt_logs_to_dfs(kt_path = '../data/atlas/models/0_hp_search_syr_G/kt/')

res.keys()

# %%
kt_tested_hps    = res['kt_tested_hps']
kt_hp_ranges     = res['kt_hp_ranges']
kt_trial_results = res['kt_trial_results']

# %%
scale_res = kt_tested_hps.merge(kt_hp_ranges, how = 'outer')
scale_res['scaled_value'] = (scale_res['value'] - scale_res['hyperparameters_min']
                            ) / (scale_res['hyperparameters_max'] - scale_res['hyperparameters_min'])
...
loss_thresh = 7
kt_trial_results = kt_trial_results.loc[(kt_trial_results.loss < loss_thresh), :]

kt_trial_results['hyperparameters'] = 'loss'

kt_trial_results['value'] = kt_trial_results['loss']

loss_min = np.nanmin(kt_trial_results['loss'])
loss_max = np.nanmax(kt_trial_results['loss'])

kt_trial_results['scaled_value'] = (kt_trial_results['loss'] - loss_min) / (loss_max - loss_min)
                     
kt_trial_results

# %%
scale_res = kt_trial_results.loc[:, ['trial_id', 'hyperparameters', 'value', 'scaled_value']].merge(scale_res, how = "outer")

# %%
scale_res = scale_res.merge(kt_trial_results.loc[:, ['trial_id', 'loss']], how = "outer")

# %%
from plotnine import *

scale_res = scale_res.sort_values('loss')

# %%
scale_res['lab'] = round(scale_res['value']*100)/100




(ggplot(scale_res, 
        aes(x = "hyperparameters", y = "orderd_id", fill = "scaled_value", label = "lab"))+
 geom_tile()#+
#  geom_text()
)
...
# %%
kt_trial_results = kt_trial_results.loc[kt_trial_results.data!= 'val', ]
kt_trial_results
...
# %%
test = res['kt_trial_results']

import matplotlib.pyplot as plt

test.loc[test.data != 'val', ]

plt.hist(test.loc[(test.loss < 1), 'loss'])
...
# %%
# test = tuner.get_best_hyperparameters(num_trials=2)
...
res2 = kt_loss_as_hp(kt_trial_results = res['kt_trial_results'],
                     considered_loss_metric = 'loss'
                    )
...
def mk_data_for_model(
    ...
    # Make sure that the needed paths exist =======================================
    if os.path.exists(kt_path)  == False:
        print('model_name='+str(model_name))
        print('Warning: needed paths were not found.\n/kt dir found: '+str(os.path.exists(kt_path)))
...
demo_folder = '0_hp_search_syr_G'
demo_output = '0_hp_search_syr_G'
