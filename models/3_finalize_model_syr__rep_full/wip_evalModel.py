...
# +
fig = go.Figure()
pointpos_intercept = [-1, -1]
pointpos_DNN =       [1, 1]
show_legend = [True,False]
set_levels = ['Train', 'Test']

for i in range(len(set_levels)):
    show_type = "Intercept"
    fig.add_trace(go.Violin(x=temp.loc[((temp.Set == set_levels[i]) & (temp.Type == show_type)), 'Set'],
                            y=temp.loc[((temp.Set == set_levels[i]) & (temp.Type == show_type)), 'Error'],
                            legendgroup=show_type, scalegroup=set_levels[i], name=show_type,
#                             name=show_type,
                            side='negative',
                            pointpos=pointpos_intercept[i], # where to position points
                            line_color=px.colors.qualitative.D3[0],
...
#                             name=show_type,
                            side='positive',
                            pointpos=pointpos_DNN[i], # where to position points
...
deciles = pd.DataFrame([10*(i+1) for i in range(9)], columns = ['Decile'])
deciles['DNN'] = q10x(test.loc[test.Type == 'DNN', 'Error'])
deciles['Intercept'] = q10x(test.loc[test.Type == 'Intercept', 'Error'])
deciles
...
temp_abs = temp
temp_abs['Error'] = abs(temp_abs['Error'])
temp_abs

# +
fig = go.Figure()
mask = ((temp_abs['Set'] == 'Test') & (temp_abs['Type'] == 'Intercept'))
fig.add_trace(go.Histogram(x=temp_abs.loc[mask, 'Error'],
                           marker_color=px.colors.qualitative.D3[0], name='Intercept',
                          cumulative_enabled=True))

mask = ((temp_abs['Set'] == 'Test') & (temp_abs['Type'] == 'DNN'))
fig.add_trace(go.Histogram(x=temp_abs.loc[mask, 'Error'], 
                           marker_color=px.colors.qualitative.D3[1], name='DNN',
                          cumulative_enabled=True))
