...
# +
run_ith_model = 0 # <-------------------------- Manually set me!
...
#     epochs = numEpochs,
#     callbacks = [
#         keras.callbacks.EarlyStopping(monitor="val_loss", patience= 7777777),
...
# +
test = tuner.get_best_hyperparameters(run_ith_model+1)
hp_set = test[run_ith_model].values

with open('./eval/hps_rank_'+str(run_ith_model)+'.json', 'w') as f:
    json.dump(hp_set, f)
...
def build_model(hp):
    ...
    # Subnetworks --------------------------------------------------------------
    ## G PCA ===================================================================
    g_num_layers = hp_set["g_num_layers"]    
    #gIn = tf.keras.Input(shape=(1725), dtype='float32', name = 'gIn') 
    gX = gIn
    for i in range(g_num_layers):
        gX = layers.Dense(
            units= hp_set["g_fc_units_"+str(i)],               
            activation = 'relu'
        )(gX)
        gX = layers.BatchNormalization()(gX)
        gX = layers.Dropout(
                rate = hp_set["g_fc_dropout_"+str(i)]   
        )(gX)   
    gOut = gX
        
    ## S =======================================================================
    s_num_layers = hp_set["s_num_layers"]
    #sIn = tf.keras.Input(shape=(21), dtype='float32', name = 'sIn')
    sX = sIn    
    for i in range(s_num_layers):
        sX = layers.Dense(
            units= hp_set["s_fc_units_"+str(i)],       
            activation = 'relu'
        )(sX)
        sX = layers.BatchNormalization()(sX)
        sX = layers.Dropout(
            rate = hp_set["s_fc_dropout_"+str(i)] 
        )(sX)
    sOut = sX
...
for ith_fold_cv in range(len(foldwise_data_list)):
    model = build_model(hp)
    epochs = numEpochs

    x_train = foldwise_data_list[ith_fold_cv]['train']['x']
    y_train = foldwise_data_list[ith_fold_cv]['train']['y']
    x_test  = foldwise_data_list[ith_fold_cv]['test' ]['x']
    y_test  = foldwise_data_list[ith_fold_cv]['test' ]['y']
    ...
    # save out history
    with open('./eval/hps_rank_'+str(run_ith_model)+'_fold_'+str(ith_fold_cv)+''+'_history'+'.json', 'w') as f:
        json.dump(history.history, f)

    # save out the model
    model.save('./eval/hps_rank_'+str(run_ith_model)+'_fold_'+str(ith_fold_cv)+'.h5')
